...
public class GeneticAlgoritm
{
    ...
    NeironNetWork bestNeironNetWork;
    ...
    public GeneticAlgoritm()
    {
        ...
        StepMutationNow = stepMutationMax;
    }

    public float StepMutationMax
    {
        get { return stepMutationMax; }
        set { stepMutationMax = value; }
    }
    ...
    bool NeedRestartGeneration(Car[] massCar)
    {
        ...
        if (maxLap == 0) StepMutationNow *= 1.1f;
        else StepMutationNow /= 2.0f;


        if (numberMutation == 18 && maxLap < 1) return true;
        if (numberMutation == 25 && maxLap < 2) return true;
        return false;
    }
    /// <summary>
    ...
    public NeironNetWork[] CreateNewCar(Car[] massCar)
    {
        ...
        bestNeironNetWork = (NeironNetWork)massNeironNetWorkBuffer[0].Clone();
    ...
    float MutationWeigth(float value, ref float delta)
    {
        ...
        //float step = stepMutationMax / ((numberMutation * numberMutation) / 2.0f);    // numberMutation
        //StepMutationNow = step;
        float step = StepMutationNow;
